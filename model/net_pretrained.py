...
def get_model(args, device):
    ...
    if (args.archi == 'resnet18'):
        ...
        model = timm.create_model('maxvit_tiny_tf_512.in1k')
        ...
        model.classifier[3] = nn.Linear(model.classifier[3], 3, bias=True)
        model.load_state_dict(torch.load(args.pretrained))
        if args.finetune:
            for (name, module) in model.named_children():
                if name == 'features' or 'avgpool':
        ...
        model.classifier[1] = nn.Linear(model.classifier[1], 3, bias=True)
        model.load_state_dict(torch.load(args.pretrained))
        if args.finetune:
            for (name, module) in model.named_children():
                if name == 'features' or 'avgpool':
